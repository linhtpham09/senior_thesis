...
class Predictor(BasePredictor):
    def setup(self):
        ...
        self.diarization = SpeakerDiarization(
            segmentation="./data/data/pyannote/segmentation/pytorch_model.bin",
            embedding="./data/data/speechbrain/spkrec-ecapa-voxceleb",
    ...
    def run_diarization(self, cache_file ="diarization_result.pkl"):
        
        if os.path.exists(cache_file):
            print(f"Loading cached diarization result from {cache_file}")
        ...
        embeddings = {
            ...
            'chunk_offset': self.diarization.segmentation_step * self.diarization._segmentation.model.specifications.duration,

        }
        
        result = self.diarization_post.process(diarization, embeddings)
            # Save result to cache
        print(f"Saving diarization result to {cache_file}")
        with open(cache_file, "wb") as f:
            pickle.dump(result, f)
        return result 

    def run_transcription(self, audio, segments, whisper_prompt=None):
        ...
        # Open a file to save transcription progress
        output_file = "transcription_chunks.json"
        with open(output_file, "w") as f:
            ...
            for seg in segments: 
                ...
                try: 
                    f.write(f"transcribing segment {format_ts(start)} to {format_ts(stop)}")
                    ...
                    # Log the error and skip the segment
                    print(f"Error while processing segment {start:.2f} to {stop:.2f}: {e}")
